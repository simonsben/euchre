...
class card:
    ...
    def is_bigger(self, other_card, trump, lead):
        ...
        if self_trump or other_trump:  # If one of the cards is trump
            if self_trump and not other_trump:
                return True  # If only self is trump
            elif not self_trump and other_trump:
                return False  # If only the other is trump
        ...
        elif self.suit == lead and other_card.suit != lead:
            return True  # Other is off-suit non-trump
        elif self.suit != lead and other_card.suit == lead:
    ...
    def is_trump(self, trump):
        if self.suit == trump:
            return True
        elif self.rank == 2 and self.suit == suit_pair.get(trump):
            return True
